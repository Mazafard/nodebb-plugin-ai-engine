...
class OllamaAdapter extends BaseProvider {
	...
	async resolveBaseUrl(config = {}) {
		const isCloud = config.ollamaUseCloud === 'on' || config.ollamaUseCloud === true;
		if (isCloud) {
			return (config.ollamaCloudUrl || 'https://api.ollama.com').replace(/\/+$/, '');
		}

		const rawUrl = (config.ollamaUrl || 'http://localhost:11434').replace(/\/+$/, '');
		if (rawUrl.includes('localhost') || rawUrl.includes('127.0.0.1')) {
			const headers = this.buildHeaders(config);
			try {
				const check = await fetch(`${rawUrl}/api/tags`, { headers, signal: AbortSignal.timeout(1000) });
				if (check.ok) return rawUrl;
			} catch (e) {
				const dockerUrl = rawUrl.replace('localhost', 'host.docker.internal').replace('127.0.0.1', 'host.docker.internal');
				try {
	...
	async testConnection(config) {
		...
		try {
			...
			return {
				ok: false,
				error: `Could not connect to Ollama at ${baseUrl}${cause}. Verify daemon/cloud URL and API key.`,
			};
		}
	}
